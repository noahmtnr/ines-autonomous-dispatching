    ...
    def step(self, action: int):
        ...
        if self.validateAction(action):
            if (action == 0):
                ...
                route = ox.shortest_path(self.graph.inner_graph, self.graph.get_nodeids_list()[self.position],
                                         self.graph.get_nodeids_list()[self.final_hub], weight='travel_time')
                route_travel_time = ox.utils_graph.get_route_edge_attributes(self.graph.inner_graph, route,
                ...
                if (self.graph.get_nodeids_list()[self.final_hub] in selected_trip['route']):
                    ...
                    step_duration = (
                                selected_trip['arrival_time_at_target_hub'] - selected_trip['departure_time']).seconds
    ...
    def compute_reward(self, done):
        ...
        # if we do not reach the final hub, reward is -1
            ...
            if self.own_ride:
                ...
                path_travelled = ox.shortest_path(self.graph.inner_graph,
                                                  self.graph.get_nodeids_list()[self.old_position],
                                                  self.graph.get_nodeids_list()[self.position], weight='travel_time')
                dist_travelled_list = ox.utils_graph.get_route_edge_attributes(self.graph.inner_graph, path_travelled,
                                                                               attribute='length')
            ...
            # if action was to share ride
                ...
                oldpath_to_final = ox.shortest_path(self.graph.inner_graph,
                                                    self.graph.get_nodeids_list()[self.old_position],
                                                    self.graph.get_nodeids_list()[self.final_hub], weight='travel_time')
                newpath_to_final = ox.shortest_path(self.graph.inner_graph,
                                                    self.graph.get_nodeids_list()[self.position],
                                                    self.graph.get_nodeids_list()[self.final_hub], weight='travel_time')
                olddist_tofinal = sum(ox.utils_graph.get_route_edge_attributes(self.graph.inner_graph, oldpath_to_final,
                                                                               attribute='length'))
                newdist_tofinal = sum(ox.utils_graph.get_route_edge_attributes(self.graph.inner_graph, newpath_to_final,
                                                                               attribute='length'))
                    ...
                    if hubsOnRoute:
                        ...
                        for hub in hubs_dict:
                            ...
                            if (str(hub) != position_str):
                                ...
                                trip = {'departure_time': departure_time, 'target_hub': hub,
                                        'arrival_time_at_target_hub': arrival_at_target_hub,
                                        'route': route_to_target_hub, 'trip_row_id': row_id}
