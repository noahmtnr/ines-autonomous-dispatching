...
class GraphEnv(gym.Env):
    ...
    def __init__(self,env_config = None):
        ...
        # time for pickup
            ...
            self.final_hub= self.graph.get_index_by_nodeid(env_config['delivery_node_id'])
            self.start_hub= self.graph.get_index_by_nodeid(env_config['pickup_node_id'])
            self.position = self.start_hub

            self.pickup_time = env_config['pickup_timestamp']
            self.time = self.pickup_time
            self.total_travel_time = 0
            self.deadline=env_config['delivery_timestamp']
            self.current_wait = 0
            manhattan_graph.setup_trips(self.pickup_time)
    ...
    def step(self, action: int):
        ...
        print("current hub: ", self.graph.get_nodeid_by_index(self.position))
        print("target hub: ", self.graph.get_nodeid_by_index(self.final_hub))
