...
input_example = {'route': [1, 2, 2, 6, 5, 5, 5, 7, 9, 15],
                 'timestamps': [datetime.datetime(2016, 5, 19, 23, 33, 46, 600000),
                                datetime.datetime(2016, 5, 19, 23, 33, 46, 600000),
                                datetime.datetime(2016, 5, 19, 23, 33, 46, 600000),
                                datetime.datetime(2016, 5, 19, 23, 33, 46, 600000),
                                datetime.datetime(2016, 5, 19, 23, 35, 46, 600000),
                                datetime.datetime(2016, 5, 19, 23, 41, 46, 600000),
                                datetime.datetime(2016, 5, 19, 23, 46, 46, 600000),
                                datetime.datetime(2016, 5, 19, 23, 56, 46, 600000),
                                datetime.datetime(2016, 5, 19, 23, 33, 46, 600000),
                                datetime.datetime(2016, 5, 19, 23, 33, 46, 600000)]}
input_example2 = {'route': [1, 4, 8, 15], 'timestamps': [datetime.datetime(2016, 5, 19, 23, 33, 46, 600000),
                                                         datetime.datetime(2016, 5, 19, 23, 33, 46, 600000),
                                                         datetime.datetime(2016, 5, 19, 23, 41, 56, 600000),
                                                         datetime.datetime(2016, 5, 19, 23, 46, 46, 600000)]}
...
real_input = {'pickup_hub': 27, 'delivery_hub': 14, 'reward': -3182.772359, 'hubs': 29,
              'route': [48, 19, 46, 47, 43, 15, 38, 24, 66, 4, 9, 62, 64, 3, 25, 58, 32, 25, 4, 37, 50, 50, 53, 67, 35,
                        15, 49, 65, 24, 17], 'time': '6:08:51.500000', 'dist': 182.77235900000005,
              'time_until_deadline': datetime.datetime(2016, 5, 20, 4, 55, 8, 500000),
              'timestamps': [datetime.datetime(2016, 5, 19, 23, 13, 24, 400000), datetime.datetime(2016,
                                                                                                   5, 19, 23, 22, 50,
                                                                                                   100000),
                             datetime.datetime(2016, 5, 19, 23, 33, 46, 600000),
                             datetime.datetime(2016, 5, 19, 23, 41, 6, 600000),
                             datetime.datetime(2016, 5, 19, 23, 51, 30, 600000), datetime.datetime(2016, 5, 20,
                                                                                                   0, 8, 15, 500000),
                             datetime.datetime(2016, 5, 20, 0, 27, 47),
                             datetime.datetime(2016, 5, 20, 0, 45, 49, 200000),
                             datetime.datetime(2016, 5, 20, 0, 54, 21, 100000),
                             datetime.datetime(2016, 5, 20, 1, 6, 52, 700000),
                             datetime.datetime(2016, 5, 20, 1, 17, 37, 200000),
                             datetime.datetime(2016, 5, 20, 1, 32, 20, 300000),
                             datetime.datetime(2016, 5, 20, 1, 40, 23),
                             datetime.datetime(2016, 5, 20, 1, 52, 2, 800000),
                             datetime.datetime(2016, 5, 20, 2, 6, 11, 900000),
                             datetime.datetime(2016, 5, 20, 2, 16, 44, 900000),
                             datetime.datetime(2016, 5, 20, 2, 31, 42, 800000),
                             datetime.datetime(2016, 5, 20, 2, 42, 43, 400000),
                             datetime.datetime(2016, 5, 20, 2, 55, 54, 800000),
                             datetime.datetime(2016, 5, 20, 3, 17, 40, 700000),
                             datetime.datetime(2016, 5, 20, 3, 32, 59, 600000),
                             datetime.datetime(2016, 5, 20, 3, 37, 59, 600000),
                             datetime.datetime(2016, 5, 20, 3, 49, 1), datetime.datetime(2016, 5, 20, 3, 56, 5, 200000),
                             datetime.datetime(2016, 5, 20, 4, 12, 37, 400000),
                             datetime.datetime(2016, 5, 20, 4, 30, 44, 300000),
                             datetime.datetime(2016, 5, 20, 4, 41, 23, 700000),
                             datetime.datetime(2016, 5, 20, 4, 53, 24, 400000),
                             datetime.datetime(2016, 5, 20, 5, 2, 9, 800000),
                             datetime.datetime(2016, 5, 20, 5, 12, 51, 500000)]}
...
def preprocess_input(inputs):
    for input in inputs:
        ...
        for i in range(len(input['route']) - 1):
            ...
            timestamps = timestamps_mapping.map_nodes_to_timestaps_to_list(route, input["timestamps"][i],
                                                                           input["timestamps"][i + 1], duration)
            for j in timestamps:
...
def buildFolium(lines, lines2, route, route2, hubs, hubs2):
    ...
    for i in range(len(hubs) - 1):
        ...
        if (hubs[i] == hubs[i + 1] and waited == 0):
            waited2 = True
            waited = 1
            folium.Marker(location=graph.get_coordinates_of_node(hubs[i]),
                          icon=folium.Icon(color='orange', prefix='fa', icon='cube'), popup=f'WAIT {waited}').add_to(
                m_events)
        ...
        # case 3: did not wait at the current hub 
            ...
            waited = 0
            folium.Marker(location=graph.get_coordinates_of_node(hubs[i]),
                          icon=folium.Icon(color='black', prefix='fa', icon='cube')).add_to(m_events)
    ...
    for i in range(len(hubs2) - 1):
        ...
        if (hubs2[i] == hubs2[i + 1] and waited == 0):
            waited2 = True
            waited = 1
            folium.Marker(location=graph.get_coordinates_of_node(hubs2[i]),
                          icon=folium.Icon(color='orange', prefix='fa', icon='cube'), popup=f'WAIT {waited}').add_to(
                m_events)
        ...
        # case 4: waited at the current hub and now moving to next hub
            ...
            waited = 0
            waited2 = False
    folium.Marker(location=start_hub_coordinates,
                  icon=folium.Icon(color='red', prefix='fa', icon='flag-checkered')).add_to(m_events)
    folium.Marker(location=final_hub_coordinates,
                  icon=folium.Icon(color='green', prefix='fa', icon='caret-right')).add_to(m_events)
...
def buildLines(routes, timestamps, color):
    ...
    for i in range(len(routes) - 1):
        element = {}
        element["coordinates"] = [DataPreProcessing.get_coordinates_of_node(routes[i]),
                                  DataPreProcessing.get_coordinates_of_node(routes[i + 1])]
...
def addTrip():
    ...
    insertIntoTrips(data.get('id'), data.get('vendor_id'), pickup_datetime, dropoff_datetime,
                    data.get('passenger_count'), pickup_longitude, pickup_latitude, dropoff_longitude, dropoff_latitude,
                    "N", trip_duration, pickup_node, dropoff_node, route_length, data.get('provider'),
