...
hub_node_ids = [42423039, 42423051, 42423296, 42423307, 42423549, 42423774, 42424025, 42424145, 42426865, 42427769,
                42427863, 42427915, 42427965, 42427968, 42427970, 42427972, 42428174, 42428863, 42428980, 42429342,
                42430253, 42430304, 42430333, 42430361, 42430375, 42432856, 42432889, 42432985, 42433058, 42433066,
                42433422, 42435684, 42436486, 42437358, 42437644, 42438809, 42439497, 42440350, 42440743, 42440798,
                42442415, 42442463, 42442475, 42442937, 42443534, 42443674, 42444457, 42445018, 424457, 370913758,
                370913779, 370914027, 370915061, 370924677, 370924957, 371188320, 371188750, 371188756, 371207282,
                371209940, 371239958, 406006393, 561035358, 561042199, 589099734, 589929417, 595295501, 595314119,
                595352904, 596775930, 1061531596, 1692433919, 1825841704, 1919595922, 2054405680, 3099327970,
                3099327976, 3579432156, 3843180751, 4145735059, 4320028826, 4779073677, 4779073680, 4886250352,
                5426969134, 5779545445, 7490266268, 9140654137, 9177424868]
...
def create_map_from_df(df_hubs, df_route=pd.DataFrame(), test_id=0):
    ...
    if not 'Rem. Distance' in df_hubs.columns:
        fig = px.scatter_mapbox(df_hubs, lat="latitude", lon="longitude", hover_name="id", color="action",
                                color_discrete_sequence=['Red', 'SaddleBrown', 'LightPink', 'OliveDrab',
                                                         'LightSlateGrey', 'LightSkyBlue'],
                                category_orders={'action': ['start', 'final', 'position', 'shared', 'book', 'hub']},
                                # size="car_hours",
                                color_continuous_scale=px.colors.cyclical.IceFire, size_max=30, zoom=11)
    else:
        fig = px.scatter_mapbox(df_hubs, lat="latitude", lon="longitude", hover_name="id", hover_data=['Rem. Distance'],
                                color="action", color_discrete_sequence=['Red', 'SaddleBrown', 'LightPink', 'OliveDrab',
                                                                         'LightSlateGrey', 'LightSkyBlue'],
                                category_orders={'action': ['start', 'final', 'position', 'shared', 'book', 'hub']},
                                # size="car_hours",
                                color_continuous_scale=px.colors.cyclical.IceFire, size_max=30, zoom=11)
...
def create_piechart(wait=0, share=0, book=0):
    ...
    fig = go.Figure(data=[
        go.Pie(labels=labels, values=values, pull=[0, 0.1, 0], text=labels, textposition='inside', textfont_size=18,
               showlegend=False, hoverinfo='text+value+percent', textinfo='value+text')])
...
app.layout = html.Div([
    html.Div(
        html.Img(src=image_path,
                 style={'width': '300px', 'display': 'inline-block', 'vertical-align': 'top', 'height': 'auto'}),
        id='image', style={'width': '300px', 'float': 'right'}),
    html.Div(
...
def render_content(tab):
    ...
    if tab == 'tab-1-example':
        return html.Div(children=[
            ...
            html.Div(children=[
                html.Div(dcc.Dropdown(['Test 1', 'Test 2', 'Test 3', 'Test 4', 'Test 5'], placeholder="Select an order",
                                      id='dropdown1'), id='dd-output-container'),
                ...
                html.Div(dcc.Graph(figure=create_chart_reduced_distance(), id='graph_distance'),
                         id='div-piechart-dist'),
    ...
    # defines layout of dynamic tab
        ...
        return html.Div(children=[
            ...
            html.Div(children=[
                ...
                html.Div(dcc.Graph(figure=create_chart_reduced_distance(), id='graph_distance2'),
                         id='div-piechart-dist2'),
...
def start_order_1(value):
    ...
    start_hub = df_test['Pickup Hub'][test_id]
    final_hub = df_test['Delivery Hub'][test_id]
    ...
    # determine by how much books and shareds have reduced the distance to the final hub
    reduced_book = 0
    reduced_share = 0
    ...
    return html.Div([
        ...
        html.P(f"Pickup Time: %s-%s-%s %s:%s" % (
        pickup_time.year, pickup_time.month, pickup_time.day, pickup_time.hour, pickup_time.minute)),
        html.P(f"Deadline: %s-%s-%s %s:%s" % (
        deadline.year, deadline.month, deadline.day, deadline.hour, deadline.minute)),
    ]), dcc.Graph(figure=create_map_from_df(df_hubs, df_route, test_id), id='my-graph'), route_string, dcc.Graph(
        figure=create_piechart(nr_wait_, nr_shared_, nr_book_), id='graph_actions'), dcc.Graph(
        figure=create_chart_reduced_distance(int(reduced_share), int(reduced_book)), id='graph_distance'),
...
def start_order_2(value):
    ...
    global start_hub
    start_hub = df_test['Hubs'][test_id][0]
    final_hub = df_test['Hubs'][test_id][-1]
...
def next_step(submit, input_value, start_dynamic=True):
    ...
    if (input_value == start_hub and entered_hub == False):
        ...
        return dcc.Graph(figure=create_map_from_df(df_hubs, df_route, test_id), id='my-graph'), dcc.Graph(
            figure=create_piechart(0, 0, 0), id='graph_actions2'), '', '', '', user_route_string
        ...
        global number_wait
        global number_book
        if current_action_string == 'Wait':
            number_wait += 1
        elif current_action_string == 'Book':
            number_book += 1
        else:
            number_share += 1
        ...
        return dcc.Graph(figure=create_map_from_df(df_hubs, df_route, test_id), id='my-graph'), dcc.Graph(
            figure=create_piechart(number_wait, number_share, number_book), id='graph_actions2'), dcc.Graph(
            figure=create_chart_reduced_distance(int(reduced_share), int(reduced_book)), id='graph_distance'), html.P(
            "Current time: %s-%s-%s %s:%s" % (
            current_time.year, current_time.month, current_time.day, current_time.hour,
            current_time.minute)), '', user_route_string
